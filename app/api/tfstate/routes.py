...
from app.deps import get_api_key_bearer_or_basic
from app.limiter import limiter
from app.models import ApiKey, BundleGroup, TerraformStateBlob, TerraformStateLock
...
async def _terraform_state_dispatch(
    ...
    if request.method == "GET":
        r = await session.execute(select(TerraformStateBlob).where(TerraformStateBlob.key == storage_key))
    ...
    if request.method == "POST":
        ...
        row = r.scalar_one_or_none()
        if row is None:
            session.add(TerraformStateBlob(key=storage_key, body=body))
        else:
            row.body = body
    ...
    if request.method == "DELETE":
        await session.execute(delete(TerraformStateBlob).where(TerraformStateBlob.key == storage_key))
        await session.commit()
        return Response(status_code=200)

    if request.method == "LOCK":
        raw = (await request.body()).decode("utf-8", errors="replace")
        lr = await session.execute(select(TerraformStateLock).where(TerraformStateLock.key == storage_key))
        ...
        session.add(TerraformStateLock(key=storage_key, lock_body=raw))
        await session.commit()
        return Response(status_code=200, content=raw, media_type="application/json")
    ...
    if request.method == "UNLOCK":
        ...
        await session.execute(delete(TerraformStateLock).where(TerraformStateLock.key == storage_key))
        await session.commit()
        return Response(status_code=200)
