...
Usage:
  envelope_run.py --envelope-url https://host[/prefix] --token TOKEN [--out FILE] [--out-format dotenv|json] -- command ...
...
def main(argv: list[str] | None = None) -> None:
    ...
    if not args.envelope_url or not args.token:
        raise SystemExit(
            "missing --envelope-url / ENVELOPE_URL or --token / ENVELOPE_ENV_TOKEN"
        )
    ...
    if not args.out:
        raise SystemExit("nothing to do: provide --out and/or a command after --")
